...
where U is set to the constant vector (0, 10).
...
where Ze, A, alpha and kappa are parameters.
...
The CMC equation is solve using finite elements in space and a central 
...
class ReactionModel:
    ...
    def __init__(self, eta):
        self.eta = eta
        self.N = len(eta)
        self.mesh = Interval(self.N - 1, 0, 1)
        ...
        self.csd_array = csd.vector().array() # deepcopy of csd
    ...
    def flamelet(self, fmean, sigma, chi):
        ...
        if (fmean < flim or fmean > (1 - flim) or  iseg < flim or iseg > (1 - flim)):
            ...
            pmf.chi = chi
            pmf.compute(0, False)
            pmf.counterflow(self.eta[1:-1], self.csd_array[1:-1])
            self.csd.vector().set_local(self.csd_array)
            count = 0
            error = 1.
            while error > 1e-8 and count < 10:
                self.A = assemble(self.J, tensor=self.A)
                self.b = assemble(self.F, tensor=self.b)
                self.bc.apply(self.A, self.b, self.u_.vector())
                self.x.zero()
                solve(self.A, self.x, self.b)
                self.u_.vector().axpy(1., self.x)
                count += 1
                error = norm(self.x)
                #print "Error = ", error
            if abs(error) > 1e-8 or isnan(error):
                self.EQ()
...
class Tinit(Expression):
    ...
    def eval(self, values, x):
        ...
        if tx in self.computed_sol:
            ...
            if self.model == 0:
                self.rx.EQ()
                sol = self.rx.u_.vector().array()
                
            elif self.model == 1:
                self.fm.eval(self.val, x)
                f, im = self.val[:]
                s = im - f * f
                self.itau.eval(self.val, x)
                chi = s * self.val[0]
                self.rx.flamelet(f, s, chi)
                sol = self.rx.u_.vector().array()
            self.computed_sol[tx] = sol
            
        for i, eta in enumerate(self.rx.eta[1:-1]):
            values[i] = sol[i + 1]
        ...
        if tx in self.computed_sol:
            ...
            if f > flim and f < (1 - flim):
                ...
                if seg > flim and seg < (1 - flim):
                    self.grad_f.eval(self.val, x)
                    dfdx, dfdy = self.val[:]
                    self.grad_im.eval(self.val, x)
                    dmdx, dmdy = self.val[:]
                    pmf.set_parameters(f, s)
                    pmf.set_fmean_gradient(dfdx, dfdy)
                    pmf.set_sigma_gradient(dmdx, dmdy)
                    pmf.chi = chi
                    pmf.compute(0, True)
                    ...
                    if seg > 0.95:
                        ...
                        #pmf.counterflow(eta[1:-1], csd)
                        pmf.CSD(eta[1:-1], self.csd)
            self.computed_sol[tx] = self.csd.copy(), self.cv.copy()
            
        for j in range(N):
            values[j] = self.csd[j]
            values[N + j] = self.cv[0, j]
            values[2 * N + j] = self.cv[1, j]
...
while error > 1e-8 and count < 10:
    J = assemble(J0)
    b = assemble(-F0)
    bc.apply(J, b, fm_.vector())
    x.zero()
    solve(J, x, b)
    fm_.vector().axpy(1., x)
    count += 1
    error = norm(x)
    print "Error in Newton solver = ", error
...
def reaction(i):
    return - (inner( (r + 1) * A * exp(- Ze / alfa - Ze * (1 - q_[i]) / (1 - alfa * (1 - q_[i])))
           * ((eta[i+1] - q_[i] / (r + 1)) * pow((1 - eta[i+1] - r * q_[i] / (r + 1)), r(0)) - kappa * pow(q_[i], r(0) + 1)), qs[i]) * dx)
           
# Set up CMC-form
CMC  =  inner(grad(q_) * U0, qs)*dx + inner(grad(q_), grad(qs))*dx

# Mixture fraction space uses finite difference discretization
for i in range(Nc-2):
    # Boundary conditions at i = 0 and i = Nc-3
    if i == 0:
        CMC -= CSD[0] / 2. * inner((q_[1] - 2 * q_[0]) / dd , qs[0])*dx
    elif i == (Nc-3):
        CMC -= CSD[Nc-3] / 2. * inner((- 2 * q_[Nc-3] + q_[Nc-4]) / dd , qs[Nc-3])*dx    
    else:
        CMC -= CSD[i] / 2. * inner((q_[i+1] - 2 * q_[i] + q_[i-1]) / dd , qs[i])*dx 
    CMC += 2. * cvx[i] * inner(grad(q_[i])[0], qs[i])*dx
    CMC += 2. * cvy[i] * inner(grad(q_[i])[1], qs[i])*dx
    CMC += reaction(i)

# Compute derivative of nonlinear form
J1 = derivative(CMC, q_, qt)   
...
b = Vector(q_.vector())
while error > 1e-8 and count < 10:
    J = assemble(J1, tensor=J)
    b = assemble(-CMC, tensor=b)
    bcmc.apply(J, b, q_.vector())
    x.zero()
    solve(J, x, b)
    q_.vector().axpy(1., x)
    count += 1
